...
export const ListadoFolios = ( { navigation,route }: Props ) => {
       ...
       <View style={{paddingBottom: 12}} ></View>
        <FlatList
          ...
          renderItem={({ item }) => 
            <TouchableOpacity 
              activeOpacity={ 0.10 }
              onPress={() => navigation.navigate("DetalleFolios", item)}
            >
               <Item item={item}/>
            </TouchableOpacity>
          }
  ...
  return (
    ...
    <View style={[CardStyles.card,styles.shadow]}>
        ...
        <View style={CardStyles.header}>
            <Text style={CardStyles.title}>{item.Folio}</Text>
        </View>
           ...
           <View style={{flex:1}}>
        {
          (item.NombreSociedad.length > 0) && (
            ...
            <Text style={{color: '#000000',fontWeight:"bold",fontSize:Platform.OS === 'ios' ? 12 : 14}}>Sociedad       : </Text>
            <Text style={{flex: 1,fontSize:Platform.OS === 'ios' ? 12 : 14,color: '#515154'}}>{item.NombreSociedad}</Text>
            </View>
...

        {
          (item.ValorFormaJuridica.length > 0) && (
            <>
            <View style={{flexDirection: 'row',justifyContent: 'center',alignItems:"center"}}>
              <Text style={{color: '#000000',fontWeight:"bold",fontSize:Platform.OS === 'ios' ? 12 : 14}}>Tipo                : </Text>
              <Text style={{flex: 1,fontSize:Platform.OS === 'ios' ? 12 : 14,color: '#515154'}}>{item.ValorFormaJuridica}</Text>
            </View>
            <View style={{paddingBottom: 3}} ></View>
            </>
          )
...

        {
          (item.Propietarios.length > 0) && (
            ...
            <Text style={{color: '#000000',fontWeight:"bold",fontSize:Platform.OS === 'ios' ? 12 : 14}}>Propietarios : </Text>
            <Text style={{flex: 1,fontSize:Platform.OS === 'ios' ? 12 : 14,color: '#515154'}}>{item.Propietarios}</Text>
            </View>
            </>
          )
        }

        {
          (item.Status.length > 0) && (
            <>
            <View style={{flexDirection: 'row',justifyContent: 'center'}}>
            <Text style={{color: '#000000',fontWeight:"bold",fontSize:Platform.OS === 'ios' ? 12 : 14}}>Estatus          : </Text>
            <Text style={{flex: 1,fontSize:Platform.OS === 'ios' ? 12 : 14,color: '#515154'}}>{item.Status}</Text>
            </View>
            </>
          )
        }
